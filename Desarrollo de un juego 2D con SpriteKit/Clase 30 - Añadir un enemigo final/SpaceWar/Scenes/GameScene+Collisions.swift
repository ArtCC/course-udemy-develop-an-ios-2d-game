...
extension GameScene {
    ...
    fileprivate func playerShotAndEnemy(_ enemy: SKSpriteNode, _ playerProjectile: SKSpriteNode) {
        ...
        case Nodes.asteroid.rawValue:
            ...
            explosion.explosion(texture: Constants.Textures.explosion, in: enemy.position)
        case Nodes.boss.rawValue:
            enemiesDestroyed += 5

            explosion.explosion(texture: Constants.Textures.bossExplosion, in: enemy.position) {
                self.endGame(isWin: true)
            }
        case Nodes.enemy.rawValue:
            enemiesDestroyed += 2

            explosion.explosion(texture: Constants.Textures.enemyExplosion, in: enemy.position)
        default:
            break
        }
    ...
    fileprivate func enemyAndPlayer(_ enemy: SKSpriteNode, _ player: SKSpriteNode) {
        ...
        case Nodes.boss.rawValue:
            enemyExplosion.explosion(texture: Constants.Textures.bossExplosion, in: enemy.position)
        case Nodes.enemy.rawValue:
    ...
    fileprivate func playerAndEnemyShot(_ player: SKSpriteNode, _ enemyProjectile: SKSpriteNode) {
        ...
        explosion.explosion(texture: Constants.Textures.playerExplosion, in: player.position) {
            self.endGame(isWin: false)
        }
